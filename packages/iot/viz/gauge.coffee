class Gauge extends UIComponent
  ...
  onCreated: ->
    ...
    # TODO: maybe this should be hardwired in?
    # The idea of where we render the gage...
    @placeholderName = "GaugeContainer"
  ...
  onRendered: ->
    ...
    @body = d3.select('#' + @placeholderName).append('svg:svg').attr('class', 'gauge').attr('width', @config.size).attr('height', @config.size)
    ...
    midValue = (@config.min + @config.max) / 2
    ...
    pointerContainer.selectAll('text').data([ midValue ]).enter().append('svg:text').attr('x', @config.cx).attr('y', @config.size - (@config.cy / 4) - fontSize).attr('dy', fontSize / 2).attr('text-anchor', 'middle').style('font-size', fontSize + 'px').style('fill', '#000').style 'stroke-width', '0px'
    # @redraw @config.min, 0
    # return
