...
class Grow {
  ...
  connect (options) {
    ...
    this.ddpclient.connect((error, wasReconnect) => {
      ...
      this.ddpclient.call(
        'Thing.register',
        [{uuid: this.config.uuid, token: this.config.token}, this.config],
        (error, result) => {
          if (error) {
            console.log(error);
          }
    ...
    this.afterConnect = (result) => {
      this.ddpclient.subscribe(
        'Thing.messages',
        [{uuid: this.config.uuid, token: this.config.token}],
        (error) => {
    ...
    // SHOULD this just emit when the thing emits?
    /**
    ...
    this.emit = (eventMessage) => {
      ...
      this.ddpclient.call(
        'Thing.event',
        [{uuid: this.config.uuid, token: this.config.token}, body],
    ...
    this.set = (key, value) => {
      ...
      if (this.ddpclient) {
        this.ddpclient.call(
          'Thing.setProperty',
          [{uuid: this.config.uuid, token: this.config.token}, key, value],
          function (error, result) {
            if (error) {
              console.log(error);
            }
