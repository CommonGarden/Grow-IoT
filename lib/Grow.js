...
class Grow extends Thing {
  ...
  registerAlerts(alerts) {
    _.each(alerts, (value, key) => {
      if (typeof value === 'object') {
        // Todo: Check if there is already a listener for this key....
        if (value.min) {
          this.on(key, (eventData) => {
            if (eventData.value < value.min) {
              if (this.alerts[key] === 'low') {
                ...
                _.extend(this.alerts, key, 'low');
                this.emit('alert', key, 'low');
                console.log('Alert: ' + key + ' low');
              }
            }
          })
        }
          ...
          this.on(key, (eventData) => {
            if (eventData.value > value.max) {
              if (this.alerts[key] === 'high') {
                ...
                _.extend(this.alerts, key, 'high');
                this.emit('alert', key, 'high');
                console.log('Alert: ' + key + ' high');
              }
            }
          })
        }
  ...
  removeAlerts(alerts) {
    _.each(alerts, (value, key) => {
      // todo: remove from this.alerts
      this.removeAllListeners(key);
    });    
  }

  // Start a start the first phase of a growfile object
  startGrow (growfile) {
  ...
  // This is to filter out bad readings... more thought needs to be put into it.
  ispH (reading) {
  ...
  // This is largely atlas scientific specific. To what degree do people want helper functions like this?
  parseEC (reading) {
  ...
  startPhase (phaseKey, growfile) {
    ...
    this.currentPhase = phases[phaseKey];
    console.log(phases[phaseKey]);
      ...
      return later.setTimeout(()=> {
        try {
          if (cycles[iteratee].targets) {
            ...
            // TODO: get targets and extend them...
