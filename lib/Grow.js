...
module.exports = class Grow extends Thing {
  ...
  calibrate (eventname, calibration_data, options) {
    ...
    // calibration_data itself doesn't have 
    // Not good enough checks that calibration_data is in the right format.
  ...
  /**
   * NEEDS REVIEW... also needs to actually use the options argument if provided
  ...
  parseAnalogpH (value, options) {
    ...
    if (this.buffers['ph'].length > 1000) {
