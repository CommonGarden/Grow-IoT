...
class Grow extends Thing {
  ...
  /**
   * Creates event listers for alerts objects in grow files.
   */
  ...
  /**
   * Schedules the phases component of a growfile. Phases can be control
   * @return     The current cycle.
   */
  parsePhases(phases) {
      ...
      return later.setTimeout(()=> {
        try {
          if (cycles[iteratee].targets) {
            // call registerTargets instead? so we can setup event listeners?
