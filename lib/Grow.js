...
class Grow {
  constructor(config) {
    ...
    if (this.config.uuid && this.config.token) {
      ...
      /**
       * Check for a state.json file to resume from. This is in case the device looses 
       * power or resets. THIS IS A HACK: someone please figure out a better solution for state.
       */
      try {
    ...
    this.thing = new Thing(this.config);
    ...
    */
    this.scheduled = {};
    this.schedule = (key) => {
      // HACK: what if they both have the same key?
      // See Thing.js for the relevant code: https://github.com/CommonGarden/Thing.js/blob/master/lib/Thing.js
    ...
    this.writeChangesToState = ()=> {
      ...
      fs.writeFile('./state.json', stringify(auth, null, 4), function (error) {
        if (error) return console.log('Error', error);
      });
    };

    /**
     * Overidden by connect method
     */
    this.log = (data) => {
      return this;
    }
    ...
    this.call = (key, options) => {
      // HACK: this section could use a lot more thinking.
      let action = this.thing.getAction(key);

      // If action disabled... we don't call it...
      // TODO: document this properly if it is useful.
      if (!action.disable) {
    ...
    */
    this.get = (property, key) => {
      // Update the thing property.
      // HACK: I'm not sure if this works with chaining.
      return this.thing.get(property, key);
    }
  }

  // TODO: break these out into seperate files when we add MQTT / REST support.
  connect (config, callback) {
    ...
    this.afterConnect = (result) => {
      ...
      this.writableStream._write = (command, encoding, callback) => {
        // Should log when a command message comes in over DDP.
        console.log(command);
